angular.module( 'rRApp' )
  .controller( 'sellerViewCtrl', function ( $scope, $state, $interval, twilioSvc, listingSvc, showingSvc, authService ) {
    setTimeout(function() {
        $(window).scrollTop(0);
    });
    var date = new Date();
    ...
    $scope.availableShowingTimes.Today.map(function(timeSlot, index) {
      if (new Date(timeSlot.timeSlot).getTime() < (rightNow + (1000 * 60 * 15))) {
    ...
    $scope.sendUrl = function() {
      ...
      * url
      * */
      var baseUrl = "http://redrover.h8.rs/?#/sellerview/" + $scope.listingId;
      var userUrl = {};
      userUrl.url = baseUrl;
      userUrl.listingId = $scope.listingId;

      /*twilioSvc.sendUrlToSeller(userUrl)
        console.log('hit sendUrlToSeller',userUrl)
        }*/
    twilioSvc.sendUrlToSeller(userUrl)
    ...
    $scope.cancelTimeslot = function(day, index, showTime) {
      $scope.showFeedback = false;
      $('.feedbackSection').removeClass('feedbackSectionOpen')
      if (showTime.status === 'pending' || showTime.status === 'confirmed') {
        var confirmed = confirm('Are you sure you want to cancel this showing?');
        if (confirmed) {
          listingSvc.cancelTimeslot({listingId: $scope.listing._id, showingId: showTime.showingId, showTime: showTime.timeSlot}).then(function(response) {
            $scope.getListing();
            return;
          })
        } else {
    ...
    $scope.uncancelTimeslot = function(day, index, showTime) {
      $scope.showFeedback = false;
      $('.feedbackSection').removeClass('feedbackSectionOpen')
      listingSvc.uncancelTimeslot({listingId: $scope.listing._id, showTime: showTime.timeSlot}).then(function(response) {
        $scope.availableShowingTimes[day][index].status = 'available';
        $scope.getListing();
      })
    };

    $scope.showFeedbackFunction = function() {
      $scope.showFeedback = !$scope.showFeedback;
      if ($scope.showFeedback) {
        $('.feedbackSection').addClass('feedbackSectionOpen')
      } else {
        $('.feedbackSection').removeClass('feedbackSectionOpen')
      }
